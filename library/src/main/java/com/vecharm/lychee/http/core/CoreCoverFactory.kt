...
class CoreCoverFactory : Converter.Factory() {
    ...
    private var delegatePartMapCover: Converter<*, *>? = null
    ...
    override fun requestBodyConverter(type: Type, parameterAnnotations: Array<Annotation>, methodAnnotations: Array<Annotation>, retrofit: Retrofit): Converter<*, RequestBody>? {
        return when {
            type == File::class.java -> FileCover(parameterAnnotations, methodAnnotations)
            parameterAnnotations.find { it is PartMap } != null -> {
                retrofit.converterFactories().filter { it != this }.find {
                    delegatePartMapCover =
                        it.requestBodyConverter(type, parameterAnnotations, methodAnnotations, retrofit); delegatePartMapCover != null
                }
                if (delegatePartMapCover == null) return null
                return MapParamsCover(parameterAnnotations, methodAnnotations, delegatePartMapCover as Converter<Any, RequestBody>)
            }
            else -> null
        }
    }
...
class MapParamsCover<T>(private val parameterAnnotations: Array<Annotation>, private val methodAnnotations: Array<Annotation>, val delegatePartMapCover: Converter<T, RequestBody>) :
    ...
    override fun convert(value: T): RequestBody {
        return if (value is File) FileCover(parameterAnnotations, methodAnnotations).convert(value)
...
class FileCover(private val parameterAnnotations: Array<Annotation>, private val methodAnnotations: Array<Annotation>) :
    Converter<File, RequestBody> {
    override fun convert(value: File): RequestBody {
        val paramFileType = (parameterAnnotations.find { it is FileType } as? FileType)?.value
            ?: (methodAnnotations.find { it is MultiFileType } as? MultiFileType)?.value ?: return create(null, value)
        return create(MediaType.parse(paramFileType), value)
    }
}
...
class CoreResponseBody(private val responseBody: ResponseBody, val response: Response? = null) : ResponseBody() {
    ...
    private var pendingReadList = ArrayList<(() -> Util)>()
    ...
    private fun parseResponseRangeFormHeader() {
        ...
        try {
            ...
            Log.e("RandomAccessFile", "Response-Header-range:$rangeStart-$rangeEnd")
